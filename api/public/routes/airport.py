...
@router.get('/lease-contract', response_model=PaginationResponse[LeaseContractResponse])
...
@router.get('/transport-stats', response_model=PaginationResponse[TransportStatsResponse])
...
@router.get('/airport-code', response_model=PaginationResponse[AirportCodeResponse])
...
@router.get('/bus', response_model=PaginationResponse[BusResponse])
...
@router.get('/daily-expect-passenger', response_model=PaginationResponse[DailyExpectPassengerResponse])
...
@router.get('/epi-gh', response_model=PaginationResponse[EpiGhResponse])
...
@router.get('/low-visibility', response_model=PaginationResponse[LowVisibilityResponse])
...
@router.get('/retail-contract', response_model=PaginationResponse[RetailContractResponse])
...
@router.get('/monthly-inout/cju', response_model=PaginationResponse[MonthlyInoutCjuResponse])
...
@router.get('/wait-time/v1', response_model=PaginationResponse[WaitTimeV1Response])
async def get_wait_time_v1(request: WaitTimeV1Request = Depends(), page: PaginationParams = Depends(), conn: aioodbc.Connection = Depends(get_connection)):
    return await service.fetch_wait_time_v1(request, page, conn)

@router.get('/wait-time/v2', response_model=PaginationResponse[WaitTimeV2Response])
async def get_wait_time_v2(request: WaitTimeV2Request = Depends(), page: PaginationParams = Depends(), conn: aioodbc.Connection = Depends(get_connection)):
    return await service.fetch_wait_time_v2(request, page, conn)
